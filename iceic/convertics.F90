...
program convertics
  ...
  character(len=120) :: dirgrd = '/scratch1/NCEPDEV/stmp4/Denise.Worthen/CPLD_GRIDGEN/angfix.2ed3a283/'
  character(len=120) :: dirsrc = '/scratch1/NCEPDEV/stmp2/Denise.Worthen/tripole/base.025.fixno360/RESTART/'
  ...
  input_file = trim(dirgrd)//'025/tripole.'//trim(fsrc)//'.nc'
  ...
  input_file = trim(dirgrd)//'100/tripole.'//trim(fdst)//'.nc'
  print *,trim(input_file)
  call nf90_err(nf90_open(trim(input_file), nf90_nowrite, ncid), 'open: '//trim(input_file))
  ...
  print *,trim(input_file)
  call getvecpair(trim(input_file), trim(dirgrd)//'025/', cosrotsrc, sinrotsrc, 'uvel', 'Bu', &
       'vvel', 'Bu', dims=(/srcnx,srcny/), vecpair=vecpairsrc)
  print *,' got vecpair'

  !print *,vecpairsrc(:,1),vecpairsrc(:,2)
  !map to Ct
  fwgt = trim(dirgrd)//'tripole.'//trim(fsrc)//'.Bu.to.Ct.bilinear.nc'
  !tmpsrc = 0.0
