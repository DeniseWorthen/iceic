...
program convertics
  ...
  ! angles on Bu
  call getfield(trim(input_file), 'angle', dims=(/srcnx,srcny/), field=angsrc)
  ...
  call nf90_err(nf90_open(trim(input_file), nf90_nowrite, ncid), 'open: '//trim(input_file))
  ! angles on Bu
  call getfield(trim(input_file), 'angle', dims=(/dstnx,dstny/), field=angdst)
  call nf90_err(nf90_close(ncid), 'close: '//trim(input_file))
  ...
  call dumpnc('testdstBu_ew.nc','vecdst', dims=(/dstnx,dstny/), nflds=2, field=tmpdst)

  ! rotate back to ij on Bu
    urot = 0.0; vrot = 0.0
    do ii = 1,dstnx*dstny
    ...
    call dumpnc('testdstBu_ij.nc','vecdst', dims=(/dstnx,dstny/), nflds=2, field=vecpairdst)
